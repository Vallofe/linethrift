...
func Usage() {
  ...
  fmt.Fprintln(os.Stderr, "  AcquireLiveTalkResponse acquireLiveTalk()")
  fmt.Fprintln(os.Stderr)
  os.Exit(0)
}
...
func main() {
  ...
  client := linethrift.NewSquareLiveTalkServiceClient(thrift.NewTStandardClient(iprot, oprot))
  ...
  case "updateLiveTalkAttrs":
    ...
    fmt.Print("\n")
    break
  case "acquireLiveTalk":
    if flag.NArg() - 1 != 0 {
      fmt.Fprintln(os.Stderr, "AcquireLiveTalk requires 0 args")
      flag.Usage()
    }
    fmt.Print(client.AcquireLiveTalk(context.Background()))
    fmt.Print("\n")
    break
  case "":
    Usage()
